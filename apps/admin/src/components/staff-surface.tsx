...
export function StaffPageHeader({ title, subtitle, actions }: PageHeaderProps) {
  return (
...
export function StaffKpiCard({ label, value, hint, railPercent, railClassName }: KpiCardProps) {
  return (
...
export function StaffSectionTitle({ title, subtitle, actions }: { title: string; subtitle?: string; actions?: React.ReactNode }) {
  return (
...
export function StaffStatusPill({ label, tone }: { label: string; tone: 'danger' | 'success' | 'warning' | 'neutral' }) {
  const className =
    tone === 'danger'
